...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'WADLにアクセスできません。RESTfulアプリケーションを再起動してこのページをリフレッシュするか、ここでURLを指定します。これは、WADLファイルを自動的に提供しないREST実装(JBoss RESTEasyなど)が原因である可能性があります。その場合、手動で事前生成するか、Jerseyライブラリに切り替える必要があります。';
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>ヘルプ・ページ</span><br/><br/><p>WADLにアクセスできません。RESTアプリケーションを再起動し、このページをリフレッシュしてください。これは、WADLファイルを自動的に提供しないREST実装(JBoss RESTEasyなど)が原因である可能性があります。その場合、手動で事前生成するか、Jerseyライブラリに切り替える必要があります。</p><p>このエラーがまだ表示され、FirebugプラグインをインストールしたFirefoxからこのページにアクセスしている場合は、<br/>ローカル・ファイルに対してFirebugを無効にする必要があります。無効にするには、Firefoxのメニュー・バーから<br/>「ツール」>「Firebug」>「Disable Firebug for Local Files」を選択します。</p>');
        }            
    },
